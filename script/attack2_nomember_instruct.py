...
def extract_top1_quality(top1_vector):
    ...
    return np.mean(top1_vector[7:])             #12最优 #[5:]
...
calib_nomember_df, test_nomember_df = train_test_split(
    df_nomember, test_size=0.2, random_state=799
)
# 将成员数据80/20划分，只保留测试部分
_, test_member_df = train_test_split(
    df_member, test_size=0.2, random_state=458
)
...
for model_name, df_roc in roc_data.items():
    # 替换文件名中的特殊字符
    filename = f"roc_data_instruct/attack2_{model_name}_0.csv"
