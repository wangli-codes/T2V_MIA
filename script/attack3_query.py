...
def load_and_aggregate_top1_multi_sample(base_path, num_samples=5) -> Dict[str, np.ndarray]:
    ...
    aggregated_data = {}
    for i in range(num_samples):
        filepath = f"{base_path}_sample_{i}_four.csv"
        if not os.path.exists(filepath):
            print(f"警告：文件不存在 {filepath}")
            continue
        ...
        for _, row in df.iterrows():
            video_id = str(row['video_id'])
            vector = np.array(ast.literal_eval(row['top1_vector']))  # top1_vector  cross_similarities_vector
            if video_id not in aggregated_data:
                aggregated_data[video_id] = []
            aggregated_data[video_id].append(vector)
    # 转换为numpy数组（shape: [num_samples, vector_dim]）
...
top1_member_base = '/data/cwy/t2v_end/methods/animatediff/frame_clip/member_25_8'  # 基础路径（不含_sample_i）
top1_nomember_base = '/data/cwy/t2v_end/methods/animatediff/frame_clip/nomember_25_8'  # 基础路径

# 加载数据（5次采样）
print("=== 加载member数据 ===")
bg_member = load_and_aggregate_multi_sample(bg_member_base, 5)
top1_member = load_and_aggregate_top1_multi_sample(top1_member_base, 1)

print("\n=== 加载nomember数据 ===")
bg_nomember = load_and_aggregate_multi_sample(bg_nomember_base, 5)
...
for i in range(1, 2):
    ...
    df_features = df_features1.groupby('sample_type', group_keys=False).apply(
            lambda x: x.sample(min(sample_size, len(x)), random_state=8697)  # 固定random_state确保可复现 7257  4836
        )
    ...
    print(f"{metrics['AUC-ROC']:.4f}   {metrics['TPR @ 1% FPR']:.4f} {metrics['balanced_acc']:.4f}")
