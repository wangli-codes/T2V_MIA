...
def extract_top1_quality(top1_vector):
    ...
    return np.mean(top1_vector)             #12最优 #[5:]
...
top1_member_base = '/data/cwy/t2v_end/methods/instructVideo/frame_clip/member_50_0.1'  # 基础路径（不含_sample_i）
top1_nomember_base = '/data/cwy/t2v_end/methods/instructVideo/frame_clip/nomember_50_0.1'  # 基础路径
...
df_features = df_features1.groupby('sample_type', group_keys=False).apply(
        lambda x: x.sample(min(sample_size, len(x)), random_state=7257)  # 固定random_state确保可复现#869
    )
