...
#     """为单采样指标（如top1_vector）加载数据，仅保留top1_vector长度为16的记录。"""
#     if not os.path.exists(filepath):
#         return None
...
#     # 检查向量长度是否为16
#     df = df[df['vector'].apply(lambda x: len(x) == 16)]
...
# print("\n--- 正在加载数据并提取原始特征 ---")
# # 数据路径
# bg_member_base = '/data/cwy/t2v_end/methods/instructVideo/multi_metrics/vector_member_50_0.1'
# bg_nomember_base = '/data/cwy/t2v_end/methods/instructVideo/multi_metrics/vector_nomember_50_0.1'
# top1_member_path = '/data/cwy/t2v_end/methods/instructVideo/frame_clip/member_50_0.1_sample_0.csv'
# top1_nomember_path = '/data/cwy/t2v_end/methods/instructVideo/frame_clip/nomember_50_0.1_sample_0.csv'
...
# #方法一：利用MLP可以达到AUC98.16%，ACC95.43%
...
#     # mean_per_dim = np.mean(bg_matrix[:, 6:], axis=1) #np.std(bg_matrix, axis=0)#
...
#     X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=302, stratify=y)
...
def load_and_aggregate_multi_sample(base_path, num_samples=5):
    ...
    for i in range(num_samples):
        ...
        for _, row in df.iterrows():
            ...
            if video_id not in aggregated_data: aggregated_data[video_id] = []
            aggregated_data[video_id].append(vector)
            # print(f"Loaded sample {i} for video_id {video_id}, vector shape: {vector.shape}")
    for video_id in aggregated_data: aggregated_data[video_id] = np.array(aggregated_data[video_id])
    return aggregated_data

def load_single_sample(filepath):
    """为单采样指标（如top1_vector）加载数据，过滤长度不足16的向量。"""
    if not os.path.exists(filepath):
        return None
    # 强制将 id 读取为字符串
    df = pd.read_csv(filepath, dtype={'id': str})
    ...
    # 解析向量并过滤长度不足16的条目
    def parse_and_filter(vector_str):
        try:
            vector = ast.literal_eval(vector_str)
            # 只保留长度为16的向量
            return vector if len(vector) == 30 else None
        except:
            return None
    
    # 应用解析和过滤
    df['vector'] = df['top1_vector'].apply(parse_and_filter)
    
    # 移除向量为空的行（即长度不足16或解析失败的）
...
def train_evaluate_mlp(features, labels, input_dim, model_name, params):
    ...
    for epoch in range(params["epochs"]):
        ...
        if val_auc > best_val_auc:
            ...
            counter += 1
            if counter >= patience and best_val_auc > 0.89:
                print(f"早停于第{epoch+1}轮，最佳验证AUC: {best_val_auc:.4f}")
                break
...
for index in range(1):
    ...
    if res['auc'] > best_results:
        ...
        for model_name, data in roc_data.items():
            ...
            csv_path = f"roc_data_mira/attack1_{model_name.replace(' ', '_')}_test.csv"
